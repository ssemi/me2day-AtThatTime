...
namespace me2dayATT.Models
{
    public class Me2API
    {
        ...
        protected string requeststr(Uri url, bool authRequest, IDictionary<string, string> param)
        {
            try
            {
                HttpWebRequest req = (HttpWebRequest)HttpWebRequest.Create(url + ".json");
        ...
        protected XmlDocument request(Uri url, bool authRequest, IDictionary<string,string> param)
        {
            try
            {
        ...
        public void GetAuth()
        {
            ...
            param.Add("akey", ConfigurationManager.AppSettings["ApplicationKey"]);
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public List<Post> getPosts(string id, int year)
        {
            DateTime date = (year != DateTime.Today.Year) ? DateTime.Today.FixNull().Replace(DateTime.Today.Year.FixNull(), year.FixNull()).ToDate() : DateTime.Today;
            //date = "2012-04-24".ToDate();
            Dictionary<string, string> param = new Dictionary<string, string>();
            param.Add("akey", ConfigurationManager.AppSettings["ApplicationKey"]);
            param.Add("from", date.ToString("yyyyMMdd"));
            param.Add("to", date.AddDays(1).ToString("yyyyMMdd"));

            string ret = requeststr(Me2Util.GetAPIUrl(Me2Util.API_METHOD_TYPE.ME2DAY_API_GET_POSTS, id), false, param);

            return JsonConvert.DeserializeObject<List<Post>>(ret); 
        ...
        public Post createPost(string id,string body, List<string> tags, int icon)
        {
            ...
            string ret = requeststr(Me2Util.GetAPIUrl(Me2Util.API_METHOD_TYPE.ME2DAY_API_CREATE_POST, id), true, param);

            return JsonConvert.DeserializeObject<Post>(ret); 
            //return JsonConvert.DeserializeObject<Post>(ret.ToJSON()); //Me2Util.FromXml<Post>(ret.SelectSingleNode("//post"));
